...
async def cm_start(callback: types.CallbackQuery, state: FSMContext):
...
async def cm_start1(callback: types.CallbackQuery, state: FSMContext):
    try:
        global Temp
        print(callback.data)
        if callback.data == '1':
...
async def cm_start2(message: types.Message, state: FSMContext):
    try:
        ...
        await state.finish()
        await bot.send_message(message.from_user.id, 'Заказ отменен!')
